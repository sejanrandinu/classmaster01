...
async function signJWT(payload, secret) {
  ...
  const stringifiedPayload = btoa(JSON.stringify({ ...payload, exp: Math.floor(Date.now() / 1e3) + 60 * 60 * 24 * 7 })).replace(/=/g, "");
...
__name(verifyEmailDomain, "verifyEmailDomain");
  ...
  if (env.RESEND_API_KEY) {
    try {
      const resendRes = await fetch("https://api.resend.com/emails", {
        ...
        body: JSON.stringify({
          from: "ClassMaster <onboarding@resend.dev>",
          // Use verified domain once set up
          to: toEmail,
          subject,
          html: htmlContent
        })
...
async function onRequest(context) {
  ...
  try {
    ...
    if (!globalThis.dbMigrated) {
      ...
      try {
        await db.prepare("ALTER TABLE exams ADD COLUMN sub_subjects_json TEXT").run();
      } catch (e) {
      }
      try {
        await db.prepare("ALTER TABLE exam_results ADD COLUMN sub_marks_json TEXT").run();
      } catch (e) {
      }
      try {
        await db.prepare("ALTER TABLE exam_results ADD COLUMN tutor_marks REAL").run();
      } catch (e) {
      }
      try {
        await db.prepare("ALTER TABLE exam_results ADD COLUMN tutor_sub_marks_json TEXT").run();
      } catch (e) {
      }
      try {
        await db.prepare("ALTER TABLE profiles ADD COLUMN whatsapp_enabled INTEGER DEFAULT 1").run();
      } catch (e) {
      }
      try {
        await db.prepare("ALTER TABLE pairing_sessions ADD COLUMN is_active INTEGER DEFAULT 1").run();
      } catch (e) {
      }
    ...
    if (path === "auth" && subPath === "register" && method === "POST") {
      ...
      const isSuperAdmin2 = email.trim().toLowerCase() === "sejanrandinu01@gmail.com";
      const role = isSuperAdmin2 ? "super-admin" : "trial";
      const approved = isSuperAdmin2 ? 1 : 0;
      const trialEndsAt = new Date(Date.now() + 7 * 24 * 60 * 60 * 1e3).toISOString();
      await db.prepare("INSERT INTO profiles (id, email, password_hash, whatsapp_number, role, is_approved, is_email_verified, verification_token, trial_ends_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)").bind(id, email, password_hash, whatsapp, role, approved, 1, null, trialEndsAt).run();
    ...
    if (path === "auth" && subPath === "resend-verification" && method === "POST") {
      ...
      await db.prepare("INSERT INTO system_notifications (id, type, recipient, content) VALUES (?, ?, ?, ?)").bind(crypto.randomUUID(), "email_verification_resend", email, verifyLink).run();
    ...
    if (path === "students" && subPath === "public-portal" && method === "GET") {
      try {
        ...
        try {
          student.subjects = JSON.parse(student.subjects_json || "[]");
        } catch (e) {
          student.subjects = [];
        }
        ...
        const examResults = (rawResults || []).map((r) => {
          ...
          let tutor_sub_marks = {};
          try {
            sub_marks = JSON.parse(r.sub_marks_json || "{}");
          } catch (e) {
          }
          try {
            tutor_sub_marks = JSON.parse(r.tutor_sub_marks_json || "{}");
          } catch (e) {
          }
        ...
        const tutes = (allTutes || []).filter(
          (t) => student.subjects && student.subjects.includes(t.subject_name)
        );
        ...
        if (classIds.length > 0) {
          ...
          pairings = (pairingsList || []).map((p) => {
            try {
              p.pairs = JSON.parse(p.pairs_json || "[]");
            } catch (e) {
              p.pairs = [];
            }
            return p;
    ...
    const logActivity = /* @__PURE__ */ __name(async (type, desc) => {
      try {
    ...
    if (path === "students") {
      if (method === "GET") {
        if (subPath === "by-id") {
          ...
          if (s) {
            try {
              s.subjects = JSON.parse(s.subjects_json || "[]");
            } catch (e) {
              s.subjects = [];
            }
            return json(s);
        ...
        const mapped = (students || []).map((s) => {
          ...
          const groups = (classes || []).filter((c) => subjects.includes(c.subject_name)).map((c) => c.whatsapp_group_url);
          return {
            ...s,
            subjects,
      ...
      if (method === "POST") {
        ...
        if (subPath === "bulk-status") {
          ...
          await db.prepare(`UPDATE students SET status = ? WHERE id IN (${placeholders}) AND user_id = ?`).bind(status, ...ids, userId).run();
        ...
        if (subPath === "bulk-delete") {
          ...
          if (!ids || !Array.isArray(ids) || ids.length === 0) return json({ error: "No student IDs provided" }, 400);
          const placeholders = ids.map(() => "?").join(",");
          await db.prepare(`DELETE FROM students WHERE id IN (${placeholders}) AND user_id = ?`).bind(...ids, userId).run();
          return json({ message: `Bulk deleted ${ids.length} students` });
        }
        await db.prepare("INSERT INTO students (user_id, student_id, name, school, grade, contact, status, subjects_json, image_url, color_theme, layout_type, show_visuals) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)").bind(userId, d.student_id, d.name, d.school, d.grade, d.contact, d.status || "Active", JSON.stringify(d.subjects || []), d.image_url || null, d.color_theme || null, d.layout_type || "standard", d.show_visuals ?? 1).run();
        await logActivity("student", `Added student ${d.name}`);
        return json({ message: "Added" });
      }
      if (method === "PUT" && subPath) {
        const d = await request.json();
        await db.prepare("UPDATE students SET student_id = ?, name = ?, school = ?, grade = ?, contact = ?, status = ?, subjects_json = ?, image_url = ?, color_theme = ?, layout_type = ?, show_visuals = ? WHERE id = ? AND user_id = ?").bind(d.student_id, d.name, d.school, d.grade, d.contact, d.status, JSON.stringify(d.subjects || []), d.image_url || null, d.color_theme || null, d.layout_type || "standard", d.show_visuals ?? 1, subPath, userId).run();
        return json({ message: "Updated" });
      }
    ...
    if (path === "tutors") {
      if (method === "GET") {
        ...
        const mapped = (results || []).map((t) => {
          let grades = [];
          try {
            grades = JSON.parse(t.grades_json || "[]");
          } catch (e) {
            grades = [];
          }
          return { ...t, grades };
      ...
      if (method === "POST") {
        const d = await request.json();
        await db.prepare("INSERT INTO tutors (user_id, name, subject, email, phone, grades_json, bank_name, bank_account_name, bank_account_number, bank_branch) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)").bind(userId, d.name, d.subject, d.email, d.phone, JSON.stringify(d.grades || []), d.bank_name, d.bank_account_name, d.bank_account_number, d.bank_branch).run();
        return json({ message: "Added" });
      }
      if (method === "PUT" && subPath) {
        const d = await request.json();
        await db.prepare("UPDATE tutors SET name = ?, subject = ?, email = ?, phone = ?, grades_json = ?, bank_name = ?, bank_account_name = ?, bank_account_number = ?, bank_branch = ? WHERE id = ? AND user_id = ?").bind(d.name, d.subject, d.email, d.phone, JSON.stringify(d.grades || []), d.bank_name, d.bank_account_name, d.bank_account_number, d.bank_branch, subPath, userId).run();
        return json({ message: "Updated" });
      }
      if (method === "DELETE" && subPath) {
    ...
    if (path === "classes") {
      ...
      if (method === "POST") {
        const d = await request.json();
        try {
          await db.prepare("INSERT INTO classes (user_id, name, tutor_name, subject_name, grade, day, class_date, start_time, end_time, fee, status, image_url, color_theme, whatsapp_group_url) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)").bind(userId, d.class_name, d.tutor, d.subject, d.grade, d.day, d.class_date || null, d.start_time, d.end_time, d.fee, d.status || "Active", d.image_url || null, d.color_theme || null, d.whatsapp_group_url || null).run();
      ...
      if (method === "PUT" && subPath) {
        const d = await request.json();
        try {
          await db.prepare("UPDATE classes SET name = ?, tutor_name = ?, subject_name = ?, grade = ?, day = ?, class_date = ?, start_time = ?, end_time = ?, fee = ?, status = ?, image_url = ?, color_theme = ?, whatsapp_group_url = ? WHERE id = ? AND user_id = ?").bind(d.class_name, d.tutor, d.subject, d.grade, d.day, d.class_date || null, d.start_time, d.end_time, d.fee, d.status, d.image_url || null, d.color_theme || null, d.whatsapp_group_url || null, subPath, userId).run();
          return json({ message: "Updated" });
        } catch (e) {
    ...
    if (path === "reminders" && subPath === "unpaid") {
      ...
      const { results: students } = await db.prepare("SELECT id, name, contact, subjects_json FROM students WHERE user_id = ? AND grade = ? AND status = 'Active'").bind(userId, cls.grade).all();
      const { results: paidStudents } = await db.prepare("SELECT student_id FROM payments WHERE user_id = ? AND class_id = ? AND month = ?").bind(userId, classId, month).all();
    ...
    if (path === "payments") {
      ...
      if (method === "POST") {
        try {
          const d = await request.json();
          const existing = await db.prepare("SELECT id FROM payments WHERE student_id = ? AND class_id = ? AND month = ? AND user_id = ?").bind(d.student_id, d.class_id, d.month, userId).first();
          if (existing) {
            return json({ error: "Duplicate Payment", details: `A payment for ${d.month} already exists for this student and class.` }, 400);
          }
          await db.prepare("INSERT INTO payments (user_id, student_id, class_id, amount, month, payment_date, payment_method, receipt_no) VALUES (?, ?, ?, ?, ?, ?, ?, ?)").bind(userId, d.student_id, d.class_id, Number(d.amount), d.month, d.payment_date || (/* @__PURE__ */ new Date()).toISOString().split("T")[0], d.payment_method, d.receipt_no).run();
    ...
    if (path === "attendance") {
      ...
      if (subPath === "upsert" && method === "POST") {
        ...
        for (const r of records) {
          await db.prepare("INSERT INTO attendance (user_id, student_id, class_id, date, status) VALUES (?, ?, ?, ?, ?) ON CONFLICT(student_id, class_id, date) DO UPDATE SET status = excluded.status").bind(userId, r.student_id, r.class_id, r.date, r.status).run();
        }
    ...
    if (path === "staff") {
      if (method === "GET") {
        if (pathParts[2] === "payments") {
          const { results: results2 } = await db.prepare("SELECT * FROM salary_payments WHERE staff_id = ? AND user_id = ? ORDER BY payment_date DESC").bind(subPath, userId).all();
    ...
    if (path === "stats" && method === "GET") {
      ...
      const colomboNow = new Date(Date.now() + 5.5 * 36e5);
      const monthPrefix = colomboNow.toISOString().substring(0, 7);
    ...
    if (path === "messages") {
      ...
      if (method === "POST") {
        const d = await request.json();
        await db.prepare("INSERT INTO messages (user_id, content, recipient_type, recipient_id, recipient_name, status) VALUES (?, ?, ?, ?, ?, ?)").bind(userId, d.content, d.recipient_type, d.recipient_id, d.recipient_name, d.status).run();
    ...
    if (path === "tutes") {
      ...
      if (method === "POST") {
        const d = await request.json();
        await db.prepare("INSERT INTO tutes (user_id, title, description, class_name, subject_name, file_url, file_type, is_active) VALUES (?, ?, ?, ?, ?, ?, ?, ?)").bind(userId, d.title, d.description, d.class_name, d.subject_name, d.file_url, d.file_type || "pdf", d.is_active ?? 1).run();
        await logActivity("tute", `Uploaded tute: ${d.title}`);
        return json({ message: "Added" });
      }
      if (method === "PUT" && subPath) {
        const d = await request.json();
        await db.prepare("UPDATE tutes SET title = ?, description = ?, class_name = ?, subject_name = ?, file_url = ?, file_type = ?, is_active = ? WHERE id = ? AND user_id = ?").bind(d.title, d.description, d.class_name, d.subject_name, d.file_url, d.file_type, d.is_active, subPath, userId).run();
        return json({ message: "Updated" });
      }
      if (method === "DELETE" && subPath) {
    ...
    if (path === "student-tutes") {
      ...
      if (method === "POST") {
        ...
        await db.prepare("INSERT INTO student_tutes (user_id, student_id, tute_id, status) VALUES (?, ?, ?, ?) ON CONFLICT(student_id, tute_id) DO UPDATE SET status = excluded.status, received_date = CURRENT_TIMESTAMP").bind(userId, student_id, tute_id, status || "Received").run();
        return json({ message: "Updated" });
      }
      if (method === "DELETE") {
        const sid = url.searchParams.get("student_id");
    ...
    if (path === "exams") {
      ...
      if (method === "POST") {
        const d = await request.json();
        const id = crypto.randomUUID();
        await db.prepare("INSERT INTO exams (id, title, class_id, subject_name, date, max_marks, sub_subjects_json) VALUES (?, ?, ?, ?, ?, ?, ?)").bind(id, d.title, d.class_id, d.subject_name, d.date, d.max_marks || 100, JSON.stringify(d.sub_subjects || [])).run();
        return json({ message: "Created", id });
      }
      if (method === "PUT" && subPath) {
        const d = await request.json();
        await db.prepare("UPDATE exams SET title = ?, subject_name = ?, date = ?, max_marks = ?, sub_subjects_json = ? WHERE id = ?").bind(d.title, d.subject_name, d.date, d.max_marks, JSON.stringify(d.sub_subjects || []), subPath).run();
        return json({ message: "Updated" });
      }
      if (method === "DELETE" && subPath) {
    ...
    if (path === "discipline") {
      ...
      if (method === "POST") {
        ...
        await db.prepare(`
                    ...
                    VALUES (?, ?, ?, ?, ?, ?, ?)
                `).bind(id, userId, Number(d.student_id), d.type, d.category, d.description, d.date || (/* @__PURE__ */ new Date()).toISOString().split("T")[0]).run();
      ...
      if (method === "PUT" && subPath) {
        ...
        await db.prepare(`
                ...
                `).bind(Number(d.student_id), d.type, d.category, d.description, d.date, subPath, userId).run();
    ...
    if (path === "pairings") {
      if (method === "GET") {
        ...
        const { results } = await db.prepare(q).bind(...p).all();
        const mappedPairings = (results || []).map((p2) => {
          let pairs = [];
          try {
            pairs = JSON.parse(p2.pairs_json || "[]");
          } catch (e) {
          }
          return { ...p2, pairs };
        });
      ...
      if (method === "POST") {
        ...
        await db.prepare(`
                    ...
                    VALUES (?, ?, ?, ?, ?, ?)
                `).bind(id, userId, Number(d.class_id), d.type, Number(d.team_size || 2), JSON.stringify(d.pairs)).run();
        return json({ message: "Pairing session saved", id });
      }
      if (method === "PUT" && subPath) {
        const d = await request.json();
        const isActiveVal = d.is_active === true || d.is_active === 1 ? 1 : 0;
        await db.prepare("UPDATE pairing_sessions SET is_active = ? WHERE id = ? AND user_id = ?").bind(isActiveVal, subPath, userId).run();
...
// ../.wrangler/tmp/pages-ft5OmW/functionsRoutes-0.20967755136987054.mjs
var routes = [
  {
    routePath: "/api/:path*",
    mountPath: "/api",
